...
```{r custom-js, echo=FALSE}
# Robust JS to switch pages
tags$script("
  window.flexDashboard_switchPage = function() {
    // Find the navbar link that contains 'Portifólio' and click it
    $('.navbar-nav a').each(function() {
      if ($(this).text().toLowerCase().indexOf('portifólio') !== -1) {
        $(this).tab('show'); // Force Bootstrap to show the tab
        $(this).click();    // Trigger the flexdashboard click event
      }
    });
  };

  $(document).on('shiny:connected', function(event) {
    Shiny.addCustomMessageHandler('switchPage', function(message) {
      flexDashboard_switchPage();
    });
  });
")
```
  ...
  fluidPage(
    ...
    conditionalPanel(
      ...
      wellPanel(
        ...
        hr(),
        div(
          p("A carteira foi gerada! Clique no botão abaixo para visualizar:"),
          actionButton("go_to_results", "Ver Portifólio Recomendado", 
                       class = "btn-success", icon = icon("arrow-right"),
                       onclick = "flexDashboard_switchPage()"),
          align = "center"
        ),
...
observeEvent(input$generate_portfolio, {
  ...
  # Troca automaticamente para a aba de resultados (usando o fallback '2' no JS)
  session$sendCustomMessage("switchPage", "2")
})

# Evento: Botão Manual de Fallback Clicado
